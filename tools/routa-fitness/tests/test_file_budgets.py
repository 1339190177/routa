...
def make_config() -> FileBudgetConfig:
    return FileBudgetConfig(
        ...
        overrides=(
            BudgetOverride(
                path="crates/routa-cli/src/commands/review.rs",
                max_lines=3010,
...
def test_load_config(tmp_path):
    ...
    assert config.overrides[0].path == "crates/routa-cli/src/commands/review.rs"
    assert config.overrides[0].max_lines == 3010
...
def test_resolve_budget_prefers_override():
    ...
    assert resolve_budget("crates/routa-cli/src/commands/review.rs", config) == (
        3010,
        "legacy hotspot",
    )
...
def test_evaluate_paths_allows_legacy_hotspot_within_override(tmp_path):
    repo_root = tmp_path
    target = repo_root / "crates" / "routa-cli" / "src" / "commands" / "review.rs"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("line\n" * 3010, encoding="utf-8")
...
def test_resolve_paths_filters_to_overrides_when_requested(tmp_path, monkeypatch):
    ...
    assert resolved == ["crates/routa-cli/src/commands/review.rs"]
