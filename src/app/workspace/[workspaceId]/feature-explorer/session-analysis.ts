...
export function buildSessionAnalysisPrompt({
  ...
  if (locale === "zh") {
    return [
      ...
      "4. 先基于这些摘要证据做你自己的综合分析，再决定是否需要回读 transcript。",
      "5. 优先基于高相关会话，总结这些会话在这个文件/功能上的共同模式。",
      "6. 把拖慢过程的问题拆开：一类是用户输入问题，例如目标不清、缺上下文、范围太大、没有先给验收标准、没有先给入口文件；另一类是环境/工具问题，例如路径引用、依赖缺失、命令执行失败。",
      ...
      "10. 如果 session 数量较多，优先直接读 Transcript Hints 里的 JSONL；先走现成 tool / script，再决定是否需要临时补脚本。",
      "10a. 如果 MCP 里有 `inspect_transcript_turns`，优先调用它来抽取真实用户 turns、目标文件相关命令、失败命令和范围漂移；如果没有，就优先运行 `node --import tsx scripts/harness/inspect-transcript-turns.ts --session-id <id> --file <path> [--feature-id <id>]`，而不是手写临时 JSONL 解析。",
  ...
  return [
    ...
    "4. Start with your own synthesis from the summarized evidence below before deciding whether transcript rereads are necessary.",
    "5. Prioritize the high-relevance sessions and summarize the recurring patterns across them for this file or feature.",
    "6. Split the slowdown analysis into user-input friction and environment/tooling friction. Do not collapse them into a single diagnosis.",
    ...
    "10. If there are many sessions, prefer reading the JSONL files from Transcript Hints directly; use the provided tool/script path before deciding whether a one-off helper script is still necessary.",
    "10a. If MCP exposes `inspect_transcript_turns`, prefer that tool for extracting real user turns, target-file commands, failed commands, and scope drift. If it does not, prefer `node --import tsx scripts/harness/inspect-transcript-turns.ts --session-id <id> --file <path> [--feature-id <id>]` before any raw JSONL parsing.",
