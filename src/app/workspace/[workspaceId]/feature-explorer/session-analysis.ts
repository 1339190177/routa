...
export function buildSessionAnalysisPrompt({
  ...
  if (locale === "zh") {
    return [
      ...
      "3. 先基于这些摘要证据做你自己的综合分析，再决定是否需要回读 transcript。",
      "4. 优先基于高相关会话，总结这些会话在这个文件/功能上的共同模式。",
      "5. 把拖慢过程的问题拆开：一类是用户输入问题，例如目标不清、缺上下文、范围太大、没有先给验收标准、没有先给入口文件；另一类是环境/工具问题，例如路径引用、依赖缺失、命令执行失败。",
      "6. 明确区分“证据支持”的判断和“你的推断”。每条结论尽量引用具体 session ID。",
      "7. 给出更好的用户输入建议，重点是下一次应该怎样一句话开场、怎样补上下文、怎样限定范围。",
      "8. 产出 2 到 4 个可直接复用的提示词模板，偏向这个文件/这个 feature 的真实场景。",
      "9. 只有在下面的摘要证据仍不足以支撑判断，或者你需要恢复真实用户开场、确认范围漂移时，才去读少量 transcript JSONL。优先读取高相关 session；如果权限被拒或读取受阻，就基于现有证据继续，并把这个限制写出来。",
      "",
      "输出格式：",
      "## 会话相关性分层",
      "## 结论",
  ...
  return [
    ...
    "3. Start with your own synthesis from the summarized evidence below before deciding whether transcript rereads are necessary.",
    "4. Prioritize the high-relevance sessions and summarize the recurring patterns across them for this file or feature.",
    "5. Split the slowdown analysis into user-input friction and environment/tooling friction. Do not collapse them into a single diagnosis.",
    "6. Distinguish clearly between evidence-backed conclusions and your own inference. Cite concrete session IDs when possible.",
    "7. Recommend better user inputs for next time: how to open the request, what context to include up front, and how to constrain the scope.",
    "8. Produce 2 to 4 reusable prompt templates tailored to this file or feature, not generic prompt-engineering advice.",
    "9. Only inspect a small number of matching transcript JSONL files if the summarized evidence is still insufficient or if you need to recover the user's true opening request or confirm scope drift. If permissions are blocked, continue with the available evidence and state that limitation explicitly.",
    "",
    "Output format:",
    "## Session Relevance",
    "## Conclusion",
