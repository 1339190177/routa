...
const KANBAN_SESSION_COPY: Record<KanbanSpecialistLanguage, KanbanSessionCopy> = {
  en: {
    ...
    noRunsInline: "No ACP runs yet",
    noRunsHistory: "No ACP runs yet.",
    ...
    emptyPaneDescription: "This card does not have a recorded ACP run yet, so the session pane is waiting for the first trigger.",
  ...
  "zh-CN": {
    ...
    noRunsInline: "还没有 ACP 运行记录",
    noRunsHistory: "还没有 ACP 运行记录。",
    noRunsHistoryHint: "等这张卡进入自动化 lane 后，每一次运行都会显示在这里。",
    emptyPaneEyebrow: "会话",
    emptyPaneTitle: "当前还没有启动 session",
    emptyPaneDescription: "这张卡还没有记录任何 ACP 运行，所以右侧 session pane 会先显示等待中的空态。",
